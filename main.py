...
def get_openai_response(prompt):
    try:
        api_key = os.getenv("OPENAI_KEY")  # Get API key from environment variable
        if not api_key:
            return "Error: OPENAI_KEY is not set"

        client = openai.Client(api_key=api_key)  # Correct client initialization
...
def ai_response():
    ...
    ai_response = get_openai_response(f"Provide a simple medical diagnosis for: {prompt}. Also suggest medicines and precautions.")
